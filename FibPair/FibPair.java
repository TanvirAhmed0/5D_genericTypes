...
public class FibPair {
    ...
    private static void nextText( Integer bigger
        ...
        Pair<Integer> current = new Pair <Integer> ( bigger, smaller);
        System.out.println(
            "The pair after "
          + current
    ...
    private static Pair <Integer> nextPairAfter( Pair <Integer> currentPair) {
        int smaller = currentPair.getFirst();
        int bigger = smaller + currentPair.getSecond();
        Pair <Integer> nextPair = new Pair<Integer>( bigger, smaller);
        return nextPair;
    }
